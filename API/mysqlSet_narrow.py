...
def updateDevice_property(imei,column,value):
    try:
        ...
        myCursor = tmp[1]
        mydbConnector = tmp[0]
        sql = "UPDATE DEVICE_PROPERTIES SET "+ column +" = %s where sn = %s"
        val = (value, imei) 
...
def insertNewWhiteList(sensor_id, device_id):
    try:
        tmp = mysqlConnect(ip, db)
        myCursor = tmp[1]
        mydbConnector = tmp[0]
        sql = """INSERT INTO WHITE_LIST (manufacturer, model, vertical, device_id, sensor_id) 
                VALUES (%s, %s, %s, %s, %s) """
        val = ('SEN', '0x78', 'watter', int(device_id), sensor_id)
        myCursor.execute(sql, val)
...
def insertDates(imei, sensor_id, hora, data, rssi):
    try:
...
def insertSensorToGraylist(imei, average, rssi, sensor_id):
    try:
        ...
        sql = """INSERT INTO GREY_LIST (average, rssi, sensor_id, device_id, timestamp) 
                VALUES (%s, %s, %s, (SELECT dp.id FROM DEVICE_PROPERTIES dp WHERE dp.imei = %s), CONVERT_TZ(NOW(), '+00:00', '+02:00'));"""
